...
export function ProductCard({ product, source }: { product: PublicProduct; source: SourcePage }) {
  ...
  return (
    <article className={`product-card product-card-${product.category.slug}`}>
      <div className="card-badges">
        {product.featured ? <span className="card-badge badge-curated">Achado</span> : <span />}
      ...
      <div className="product-card-body">
        ...
        <div className="price-block">
          <span className="price-label">Preço observado:</span>
          {product.latestPrice ? (
            <p className="price">
            ...
            <p className="muted">Consulte o preço na loja</p>
          )}
        </div>
        <div className="card-cta">
          <Link className="btn-details" href={`/produto/${product.slug}`}>
            Detalhes
          </Link>
          <a className="button btn-offer" href={offerHref(product.slug, source)}>
...
export function ProductGrid({
  products,
  source,
  emptyMessage = "Ainda não encontramos um achado aqui.",
}: {
  ...
  if (!products.length) {
    return (
      <div className="empty" role="status">
        <h2>Nenhum produto por enquanto</h2>
        <p>{emptyMessage}</p>
  ...
  return (
    <div className="product-grid">
      {products.map((product) => (
        <ProductCard product={product} source={source} key={product.id} />
      ))}
    </div>
  );
}
...
export function PriceDisclosure() {
  return (
    <p className="disclosure">
      <sup>*</sup> Preço de referência observado, não preço em tempo real. O valor pode mudar e a disponibilidade
      pertence ao marketplace. O preço final é o apresentado pelo marketplace; a Garimora não processa a compra.
    </p>
  );
}
...
export function CatalogUnavailable() {
  return (
    <div className="empty" role="status">
      ...
      <p>Não foi possível carregar os achados agora. Tente novamente em instantes.</p>
    </div>
  );
}
